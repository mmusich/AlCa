...
for ds in DATASETS:
    results = commands.getstatusoutput("das_client --limit=0 --query='run dataset="+ds+"'")[1] #.splitlines()
...
for run in RUNS:
    ...
    for ds in DATASETS:
        if run in the_RUN_DS_MAP[ds][0]:
            blocks=commands.getstatusoutput("das_client --limit=0 --query='block dataset="+ds+" run="+run+"'")[1]
            print "run",run,"is found in",ds,"in block",blocks
            theBlocks_to_subscribe.append(blocks)
